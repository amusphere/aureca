...
class TestAIChatUsageE2EIntegration:
    ...
    def test_complete_user_journey_within_limits(
    ...
    def test_complete_user_journey_reaching_limit(
        self, client: TestClient, session: Session, test_user: User
    ):
        ...
        try:
            with patch(
                ...
                ai_response = client.post(
                    "/api/ai/process", json={"prompt": "Should fail"}
                )
    ...
    def test_daily_reset_user_journey(
        self, client: TestClient, session: Session, test_user: User
    ):
    ...
    def test_free_plan_user_complete_journey(self, client: TestClient, test_user: User):
        ...
        try:
            with patch(
                ...
                ai_response = client.post(
                    "/api/ai/process", json={"prompt": "Should be blocked"}
                )
    ...
    def test_error_recovery_user_journey(
        self, client: TestClient, session: Session, test_user: User
    ):
    ...
    def test_api_consistency_across_endpoints(
        self, client: TestClient, session: Session, test_user: User
    ):
        ...
        try:
            with patch(
                ...
                assert (
                    increment_data["remaining_count"] == get_data["remaining_count"] - 1
                )
        finally:
            self._cleanup_auth()

    def test_high_usage_scenario(
        self, client: TestClient, session: Session, test_user: User
    ):
                ...
                with patch(
                    ...
                    for i in range(9):
                        response = client.post(
                            "/api/ai/process", json={"prompt": f"Prompt {i+1}"}
                        )
                        assert response.status_code == 200
                    ...
                    over_limit_response = client.post(
                        "/api/ai/process", json={"prompt": "Over limit"}
                    )
                    assert over_limit_response.status_code == 429
        finally:
            self._cleanup_auth()

    def test_usage_tracking_accuracy(
        self, client: TestClient, session: Session, test_user: User
    ):
